...
and acct_type_desc != 'Tix Pro Seller'
and tm_acct_id not in(-1,-2)
and full_date>DATEADD(DAY,-7,GETDATE())
)
select tm_acct_id,zip,country
from table1
'''
...
a=to_integer(today)
dfa= pd.DataFrame(index=['2017_season','%s to %s'%(b,a)], columns=['repeat_purchased','>75 miles','repeat_purchased_and_>75_ miles','other','total'])
...
dfb= pd.DataFrame(index=['2017_season','%s to %s'%(b,a)], columns=['repeat_purchased','>75 miles','repeat_purchased_and_>75_ miles','other'])
...
dfc.to_csv('Rockettes_stats_%s.csv'%a)
'''
import smtplib
import mimetypes
