...
ticket_type_query = '''
SELECT DISTINCT tm_acct_id, ads_source, ticket_product_description, tm_price_code,
...
#ticket_data = pd.merge(ticket_data, labels, on = ['tm_acct_id'], how = 'left').drop(['ticket_product_description','description_level'], axis = 1).rename(columns = {'category':'ticket_product_description'})
...
mobile_by_group = mobile_data.groupby(['category','customer_account_id','ads_source']).max()[['attendance_mobile_flag']].reset_index().groupby(['ads_source','category']).mean()['attendance_mobile_flag']
...
mobile_data_query = '''
SELECT e.tm_event_date::DATE, a.attendance_mobile_flag, a.manifest_seat_id, c.tm_acct_id, c.email_address, pc.ticket_type_price_level, e.ads_source, tm_price_code
...
join ads_main.d_price_code pc on pc.price_code_id=a.price_code_id
'''
mobile_data2 = pd.read_sql(mobile_data_query, engine, parse_dates = ['tm_event_date'])
#mobile_data2.ix[mobile_data2['ticket_type_price_level'] == 'Lounges', 'ticket_type_price_level'] = 'Groups'
#mobile_data2.ix[mobile_data2['ticket_type_price_level'] == 'Pick Plan', 'ticket_type_price_level'] = 'Mini Plan'
#mobile_data2.groupby(['ads_source','tm_acct_id']).max()['attendance_mobile_flag'].mean()

mobile_data2[['tm_acct_id','ads_source','ticket_type_price_level']].drop_duplicates().groupby('ticket_type_price_level').count()

test = pd.merge(ticket_data[['tm_acct_id','ads_source','ticket_product_description','tm_price_code']].drop_duplicates(), mobile_data2[['tm_acct_id','ads_source','ticket_type_price_level','tm_price_code']].drop_duplicates(), on = ['tm_acct_id','ads_source','tm_price_code'], how = 'left')
test[pd.isnull(test['ticket_type_price_level'])]['tm_price_code'].value_counts()
test2 = pd.merge(ticket_data[['tm_acct_id','ads_source','ticket_product_description','tm_price_code']].drop_duplicates(), mobile_data2[['tm_acct_id','ads_source','ticket_type_price_level','tm_price_code']].drop_duplicates(), on = ['tm_acct_id','ads_source','tm_price_code'], how = 'right')
test2[pd.isnull(test2['ticket_product_description'])]['tm_price_code'].value_counts()
test3 = pd.merge(ticket_data[['tm_acct_id','ads_source','ticket_product_description','tm_price_code']].drop_duplicates(), mobile_data2[['tm_acct_id','ads_source','ticket_type_price_level','tm_price_code']].drop_duplicates(), on = ['tm_acct_id','ads_source','tm_price_code'])
